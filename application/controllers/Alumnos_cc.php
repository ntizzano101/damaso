...
//Seguir Aca!!
       ...
       $contador=0;
		...
		if($factura->cae==""){
			$d0=shell_exec("php /var/www/html/facturaelectronica/index.php ".$id_pago ." ");
			sleep(2);
			if(trim($d0)!=""){
		...
		if($factura->barranro==""){
			$d1=shell_exec("php /var/www/html/facturaelectronica/ArchivosyCodBar.php ".$id_pago ." ");
			sleep(2);
			if(trim($d1)!=""){
				$factura=$this->alumnos_cc_model->borrarpago($id_pago);				
     ...
     if($contador==4){
		$factura=$this->alumnos_cc_model->borrarpago($id_pago);
		die("id ".$id_pago . "POR EL MOMENTO NO ES POSIBLE FACTURAR ESTAMOS TRABAJANDO");
	...
	public function recibo($id=''){
			...
			$otros=$this->alumnos_medios_pagos_model->cc_pago_iva($id);	
			$data=array(
			'recibo'=>$recibo,
			'items'=>$items,
			'otros'=>$otros
