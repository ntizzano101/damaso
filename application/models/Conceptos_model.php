...
class Conceptos_model extends CI_Model {
        ...
        {
				$query=$this->db->get_where('conceptos',"fechabaja is null");
				return $query->result();
        }
		public function buscar($p)
        {
				$query=$this->db->get_where('conceptos',array("nombre like"=>$p.'%',"fechabaja is null"));
       ...
       public function listarconceptosalumno($p){
		   ...
		   LEFT join alumnos_conceptos on alumnos.id=alumnos_conceptos.id_alumno 
		   LEFT join conceptos on conceptos.id=alumnos_conceptos.id_concepto where apellido like ? group by alumnos.id,alumnos.apellido,alumnos.nombre ";
            ...
            public function conceptosParaAgregar($p)
        {
		    ...
		    where 
		     alumnos_conceptos.id_concepto IS NULL and conceptos.fechabaja is null
