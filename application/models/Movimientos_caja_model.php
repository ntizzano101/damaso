...
public function listar($d,$h){
					$sql="
					SELECT T1.* from (		
					SELECT cm.id, cm.monto, cm.descripcion, cm.tipo, date_format( cm.fecha, '%d/%m/%Y' ) AS fechaM, ifnull( cc.nombre, 'Sin Concepto' ) AS concepto,cm.fecha as fecha,cm.caoba as tipo2
					...
					WHEN '13'
					THEN 'H'
					END , date_format( cpm.fecha_acreditacion, '%d/%m/%Y' ) , 'Cobro Cuota',cpm.fecha_acreditacion,cpm.tipo_pago
...
public function listar_una_caja($id){
					...
					(
						...
						AS T1
						ORDER BY descripcion,nombreA,fecha, id
					";
					//die($sql);
